...
<style>
:root{color-scheme:dark;--bg:#0d1117;--panel:#161b22;--panel2:#21262d;--border:#30363d;--text:#e6edf3;--muted:#8b949e;--accent:#58a6ff;--add:#aff5b4;--addbg:#033a16;--del:#ffdcd7;--delbg:#67060c;--warn:#d29922;--ok:#3fb950}
*{box-sizing:border-box}html,body{height:100%;margin:0}body{font:14px/1.45 system-ui,-apple-system,Segoe UI,sans-serif;background:var(--bg);color:var(--text);overflow:hidden}
button,input{font:inherit}.app{height:100%;display:grid;grid-template-columns:330px 1fr}.sidebar{min-width:0;background:var(--panel);border-right:1px solid var(--border);display:flex;flex-direction:column}.brand{padding:18px 16px 12px;border-bottom:1px solid var(--border)}.brand h1{font-size:16px;margin:0 0 4px}.brand p{margin:0;color:var(--muted);font-size:12px}.search{padding:12px;border-bottom:1px solid var(--border)}.search input{width:100%;padding:8px 10px;border:1px solid var(--border);border-radius:6px;background:var(--bg);color:var(--text)}.tree{overflow:auto;padding:8px;flex:1}.tree details{margin:1px 0}.tree summary{cursor:pointer;color:var(--muted);padding:4px 6px;user-select:none}.tree .children{padding-left:14px}.file{width:100%;display:flex;gap:8px;align-items:center;text-align:left;border:0;border-radius:6px;padding:6px 8px;color:var(--text);background:transparent;cursor:pointer}.file:hover{background:var(--panel2)}.file.active{background:#1f6feb33;color:#fff}.file .name{white-space:nowrap;overflow:hidden;text-overflow:ellipsis;flex:1}.badge{font-size:10px;color:var(--muted);border:1px solid var(--border);border-radius:999px;padding:1px 6px}.main{min-width:0;display:flex;flex-direction:column}.toolbar{min-height:70px;padding:12px 16px;border-bottom:1px solid var(--border);background:var(--panel);display:flex;align-items:center;gap:12px;flex-wrap:wrap}.path{font:600 14px ui-monospace,SFMono-Regular,Consolas,monospace;min-width:240px;flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.controls{display:flex;gap:6px;align-items:center}.controls button{border:1px solid var(--border);background:var(--panel2);color:var(--text);padding:6px 10px;border-radius:6px;cursor:pointer}.controls button:hover{border-color:var(--muted)}.controls button.active{background:#1f6feb;border-color:#58a6ff}.meta{width:100%;color:var(--muted);font-size:12px;display:flex;gap:12px;flex-wrap:wrap}.diff{overflow:auto;flex:1;font:13px/1.45 ui-monospace,SFMono-Regular,Consolas,Liberation Mono,monospace}.line{display:grid;grid-template-columns:60px 60px 24px minmax(max-content,1fr);min-height:20px;border-left:3px solid transparent}.line:hover{background:#ffffff08}.ln{padding:1px 8px;text-align:right;color:#6e7681;user-select:none;border-right:1px solid #21262d}.prefix{padding:1px 6px;text-align:center;color:var(--muted);user-select:none}.code{padding:1px 10px;white-space:pre}.remove,.remove_note,.filtered_remove{background:var(--delbg);color:var(--del);border-left-color:#f85149}.add,.add_note,.filtered_add{background:var(--addbg);color:var(--add);border-left-color:#3fb950}.hunk,.title,.section,.selector,.selector_selected,.test_file_header,.dev_file_header,.file_header{background:#1f6feb22;color:#a5d6ff;font-weight:600}.filtered,.filtered_header,.handled{background:#8957e522;color:#d2a8ff}.error{background:var(--delbg);color:var(--del);font-weight:700}.empty{padding:48px;text-align:center;color:var(--muted)}.footer{padding:6px 12px;border-top:1px solid var(--border);background:var(--panel);color:var(--muted);font-size:11px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
@media(max-width:800px){.app{grid-template-columns:240px 1fr}.line{grid-template-columns:46px 46px 20px minmax(max-content,1fr)}}
</style>
</head>
<body>
<div class="app">
...
let mode='focused',selected=0,visible=snapshot.files.slice();
...
function renderDiff(){const file=visible.length?snapshot.files[selected]:null;if(!file){$('path').textContent='No matching files';$('diff').innerHTML='<div class="empty">No files match the search.</div>';return}const view=file[mode];$('path').textContent=file.path;$('focused').classList.toggle('active',mode==='focused');$('raw').classList.toggle('active',mode==='raw');const hidden=view.noiseHidden+view.whitespaceHidden+view.orderHidden;$('meta').textContent=`${file.status} · ${view.visibleChanges} visible change${view.visibleChanges===1?'':'s'}${mode==='focused'?` · ${hidden} hidden · ${view.handled} handled`:''}`;$('footer').textContent=`Snapshot ${snapshot.generatedAt} · ${snapshot.gitStatus} · reopen from the terminal to refresh`;
const host=$('diff');host.replaceChildren();if(!view.lines.length||view.visibleChanges===0&&mode==='focused'){const e=document.createElement('div');e.className='empty';e.textContent=mode==='focused'?'No visible Focused Diff changes. Switch to Raw to inspect all literal differences.':'No literal differences.';host.append(e);return}for(const line of view.lines){const row=document.createElement('div');row.className='line '+line.kind;const tl=document.createElement('div');tl.className='ln';tl.textContent=line.testLine??'';const dl=document.createElement('div');dl.className='ln';dl.textContent=line.devLine??'';const p=document.createElement('div');p.className='prefix';p.textContent=prefixFor(line.kind);const code=document.createElement('div');code.className='code';code.textContent=line.text;row.append(tl,dl,p,code);host.append(row)}host.scrollTop=0;host.scrollLeft=0}
...
$('search').addEventListener('input',render);$('prev').onclick=()=>move(-1);$('next').onclick=()=>move(1);$('focused').onclick=()=>{mode='focused';renderDiff()};$('raw').onclick=()=>{mode='raw';renderDiff()};document.addEventListener('keydown',e=>{if(e.target===$('search'))return;if(e.key==='['){move(-1);e.preventDefault()}else if(e.key===']'){move(1);e.preventDefault()}else if(e.key==='f'){mode='focused';renderDiff()}else if(e.key==='r'){mode='raw';renderDiff()}else if(e.key==='/'){$('search').focus();e.preventDefault()}});render();
</script>
</body>
</html>"""
